...
{
  const s = newSlide();
  header(s, "今後の進め方", "風力×蓄電池の初期分析の帰結として、「対象」と「手法」を次回までに確定する");

  secLabel(s, 0.6, 1.4, "発見① 風力は、スポット市場を通じては蓄電池のオプション価値をほとんど動かさない可能性", 11.9);
  bullets(s, [
    { t: "価格カーブを平行移動させるだけで日内の「形」を作らない（変動の約3/4が1日超の長周期。日内帯は1割弱 — 太陽光の65%と対照的）", sub: true },
    { t: "実測でも風力大の日はスプレッド微減（冬−19%）。導入量を3倍まで振っても、価値は床（0.01円）経由の緩やかな増加に留まる", sub: true },
    { t: "→ 系統用（FTM）のスポット価値の源泉は夕方スパイクと床の形成であり、風力単独ではどちらも大きく動かせない", sub: true, b: true },
  ], 0.75, 1.8, 11.9, 1.55, { size: 11.5, gap: 5 });

  secLabel(s, 0.6, 3.4, "発見② 一方で、併設（BTM）の価値算出には固有の意義がある可能性", 11.9);
  bullets(s, [
    { t: "出力制御が立ち上がり（FY2025 0.1% → FY2026上期 1.35%）、その1割強はエリア価格が高い時間に発生 ＝ エリア価格に映らないローカル制約由来の価値", sub: true },
    { t: "エリア価格ベースの現行分析はBTM価値をゼロと計上（＝蓄電池価値の総体としては下限）。抑制回避価値は九州並み8%で約2,500円/kW風力-年へ", sub: true },
    { t: "→ 対象を「系統用（FTM）＋併設（BTM）」の二本立てへ拡張する価値がある", sub: true, b: true },
  ], 0.75, 3.8, 11.9, 1.55, { size: 11.5, gap: 5 });

  secLabel(s, 0.6, 5.4, "進め方: 手法は先行研究の整理と接続し、次回に確定", 11.9);
  bullets(s, [
    { t: "コア文献41本の整理は完了済み（均衡系: Butters 2025・Karaduman等／手法系: 完全予見LP＋ヘアカット・打ち切り分位点・実物オプション）", sub: true },
    { t: "次回まで: ①FTM＝蓄電池競合を内生化したπ(K)曲線→均衡容量の初回推計 ②BTM＝抑制イベントの分解と容量制約下の回避可能率 ③共通基盤＝価格過程の改良", sub: true },
  ], 0.75, 5.8, 11.9, 1.0, { size: 11.5, gap: 5 });

  s.addShape(pres.ShapeType.rect, { x: 0.6, y: 6.75, w: 12.13, h: 0.5, fill: { color: C.tint } });
  s.addText("次回のゴール: 先行研究を踏まえた手法・対象の確定（研究計画の凍結）", { x: 0.85, y: 6.75, w: 11.6, h: 0.5, fontSize: 11.5, bold: true, color: C.teal, fontFace: FB, margin: 0, valign: "middle" });
}
